...
(function () {
  ...
  widgetStyles.textContent = `
        ...
        /* --- STYLES FOR CHAT WINDOW --- (Unchanged) */
        ...
        /* ... All other existing chat window styles (.chat-header, .chat-body, etc) ... */
        /* ... (styles omitted for brevity, but they are still here) ... */
        ...
        .chat-assist-widget .user-registration.chat_active { display: block; }
        ...
        /* --- MODIFICATION: STYLES FOR NEW PROMPT POPUP --- */
        .chat-assist-widget .chat-prompt-popup {
            position: fixed;
            bottom: 85px; /* Positioned just above the 56px launcher + 20px bottom */
            z-index: 1000;
            background: var(--chat-color-surface);
            border-radius: var(--chat-radius-md);
            box-shadow: var(--chat-shadow-lg);
        ...
        .chat-assist-widget .chat-prompt-popup.visible {
            opacity: 1;
            transform: translateY(0);
            visibility: visible;
        }
        .chat-assist-widget .chat-prompt-popup.right-side { right: 20px; }
        .chat-assist-widget .chat-prompt-popup.left-side { left: 20px; }

        .chat-assist-widget .chat-prompt-message {
            font-size: 14px;
            color: var(--chat-color-text);
            line-height: 1.5;
            font-weight: 500;
        }
        .chat-assist-widget .chat-prompt-close {
            position: absolute;
            top: 6px;
            right: 6px;
  ...
  // Merge user settings with defaults (Unchanged)
  ...
  // Create welcome screen with header (Includes hidden page_url input)
  ...
  // Create chat interface (Unchanged)
  ...
  // Create toggle button (Unchanged)
  ...
  // --- MODIFICATION: Create proactive chat prompt ---
  ...
  chatPrompt.innerHTML = `
        <div class="chat-prompt-message">
            Hey, we are here to help you. You may chat with us.
        </div>
  ...
  widgetRoot.appendChild(chatPrompt); // --- MODIFICATION: Added prompt to DOM ---
  ...
  // Helper function to generate unique session ID (Unchanged)
  ...
  // Create typing indicator element (Unchanged)
  ...
  // Function to convert URLs in text to clickable links (Unchanged)
  ...
  // Helper functions for localStorage chat history (Unchanged)
  ...
  // Show registration form and populate page_url (Unchanged)
  ...
  // Send a message to the webhook (Includes pageUrl) (Unchanged)
  ...
  // Auto-resize textarea as user types (Unchanged)
  ...
  // --- MODIFICATION: Updated loadChatState with 10-sec prompt ---
  ...
  // --- MODIFICATION: Launcher click also hides prompt ---
  ...
  // --- MODIFICATION: Event listeners for new prompt ---
  // Close button on the prompt
  chatPromptClose.addEventListener("click", () => {
    chatPrompt.classList.remove("visible");
  });
